    ...
    for friday in list_fridays:
        ...
        if options_chain:
            ...
            for i in range(length_of_chain):
                ...
                annualized_return = calculate_annualized_return(strike_price, bid_price, dte)
                percent_change = calculate_percent_change(strike_price, underlying_price)
                ...
                if contract_type == 'put':
                    ...
                    logger.debug(f"Date: {expiration_date} annualized: {annualized_return} delta: {delta} desired: {desired_delta}")
                    if annualized_return >= 30 and delta >= -0.20:
                        options.append(this_option)
                else:
                    desired_delta = get_desired_delta('call', dte)
                    logger.debug(f"C{strike_price} annualized return: {annualized_return} delta: {delta} desired: {desired_delta}")
...
def find_rolls(ticker, type, current_strike, current_expiration, number_of_weeks=2, debug=False):
    options = []
    ...
    for friday in list_fridays:
        ...
        if options_chain:
            ...
            for i in range(length_of_chain):
                ...
                annualized_return = calculate_annualized_return(strike_price, roll_price, dte)
                percent_change = calculate_percent_change(strike_price, underlying_price)
                ...
                if contract_type == 'put':
                    # desired_delta = get_desired_delta('put', dte)
                    logger.debug(f"   Date: {expiration_date} Strike: {strike_price} price: {bid_price} diff: {roll_price} annualized: {annualized_return} delta: {delta}")
                    options.append(this_option)
                else:
                    desired_delta = get_desired_delta('call', dte)
                    logger.debug(f"   Date: {expiration_date} Strike: {strike_price} price: {bid_price} diff: {roll_price} annualized: {annualized_return} delta: {delta} desired delta: {desired_delta}")
                    options.append(this_option)
